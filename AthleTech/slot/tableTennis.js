...
// Function to set up booking for table tennis
function setupTableTennisBooking() {
    document.getElementById('bookTableTennisButton').addEventListener('click', function() {
        const date = document.getElementById('date').value;
        const time = document.getElementById('time').value;
        const court = document.getElementById('tableTennisCourt').value;
        ...
        if (isCourtBooked('tableTennis', date, court, endTime)) {
            alert('This court is already booked for the selected time. Please choose another one.');
        } else {
            bookCourt('tableTennis', date, court, time, endTime);
            updateBookedSlots();
...
function updateBookedSlots() {
    const bookedTableTennisSlotsList = document.getElementById('bookedTableTennisSlots');
    bookedTableTennisSlotsList.innerHTML = '';

    for (const court in SbookedSlots.tableTennis) {
        SbookedSlots.tableTennis[court].forEach(booking => {
            const li = document.createElement('li');
            li.textContent = `${booking.date} - ${court} - ${booking.time} (Booked until ${new Date(booking.endTime).toLocaleTimeString()})`;
            bookedTableTennisSlotsList.appendChild(li);
        });
    }
}
